...
### Функции работы с БД
    ...
    def create_tables(_connection):
        with _connection:
            ...
            cur.execute('''CREATE TABLE IF NOT EXISTS employees (name text, card_number text, role text)''')
    ...
    def _update_employees(self):
        ...
        sql_query = '''INSERT INTO employees (name, card_number, role)
                                        VALUES (?, ?, ?)'''
        values = []
        for employee in self._employees:
            if isinstance(employee, Employee):
                values.append((employee.name, employee.card_number, employee.role))
            else:
    ...
    def _update_employee_connections(self):
        ...
        sql_query = '''INSERT INTO employee_connections (card_number, team_num, team_date)
                                        VALUES (?, ?, ?)'''
        values = []
        for ec in self._employee_connections:
            if isinstance(ec, Employee_connection):
                values.append((ec.employee.card_number, ec.team.num, ec.team.date))
            else:
